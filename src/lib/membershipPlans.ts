...
/** Current checkout policy first, followed by frozen legacy membership terms. */
export const SUPPORTED_YOUTH_FAMILY_DISCOUNT_PERCENTAGES = [
...
export const CHECKOUT_DOCUMENTS = {
  ...
  adultConditioningAddendum: {
    ...
    publicUrl: "/legal/products/ZAF-CONDITIONING-ADDENDUM-2026-09-01-01.txt",
    contentType: "text/plain; charset=utf-8",
    hashCovers: "UTF-8 bytes of content",
    sha256: "b91f7fd256bca8ca6bca8d020519c5e298b167a8b98ea039df213bd4bc71fc19",
    content: "Adult Conditioning Only Membership Product Addendum\n\nVERSION DATE\n1 September 2026\n\nDOCUMENT ID\nZAF-CONDITIONING-ADDENDUM-2026-09-01-01\n\nCONTRACTING ENTITY\nZERO ALPHA FITNESS LTD · company number 15978998\n\nTRADING NAME\nZero Alpha Fitness\n\nCONTACT\nsupport@zeroalphafitness.co.uk\n\n1. How this addendum applies\n\nThis addendum supplements the Membership Terms, Cancellation, Refund and Cooling-off Policy, Privacy Notice, and Adult Participant Waiver shown at checkout. If this addendum conflicts with a general description of app access or the Adult Conditioning Only Membership, this addendum applies to that product only. All statutory rights remain unaffected.\n\n2. Product and price\n\nThe Adult Conditioning Only Membership is a rolling monthly adult membership priced at £30 per month. It has no joining fee, free trial or minimum term. Stripe shows the amount due today, the first payment date and the recurring billing schedule before the member confirms payment. The general membership cancellation, first-of-month billing, failed-payment, refund and cooling-off provisions continue to apply.\n\n3. Weekly class entitlement\n\nWhile the membership and its payment entitlement are active, the named member may book a maximum of two eligible Conditioning classes in each Europe/London Monday-to-Sunday week.\n\nThe eligible recurring timetable at the version date is:\n\n• Monday at 06:00.\n\n• Tuesday at 18:00.\n\n• Thursday at 18:00.\n\n• Friday at 05:30.\n\nThe member chooses from eligible classes actually published in the live schedule. The member may choose different eligible classes from week to week; checkout does not reserve fixed recurring slots. A booking counts against the week containing that class's scheduled start time in Europe/London.\n\nEvery booking remains subject to the class being published, open, suitable and not full. The membership does not guarantee a place in a particular class. The app and server enforce the same weekly limit and eligible timetable.\n\n4. Changes, cancellations and attendance\n\nIf the member cancels an eligible booking through the authorised booking flow before it is treated as attended or a no-show, that booking is released from the same week's two-class allowance and the member may book another eligible class in that week, subject to capacity.\n\nA class recorded as attended or as a no-show continues to use one place from that week's allowance. A place is not carried forward, exchanged for money, transferred to another person or converted into a reusable credit.\n\nIf Zero Alpha Fitness cancels a class, the cancelled booking should not consume the member's allowance. The member may choose another eligible class in the same week, subject to capacity. Any wider remedy required by consumer law or the general Membership Terms remains available.\n\n5. Limited app access\n\nAfter the first required payment succeeds, and while the subscription remains eligible, this membership provides limited Zero Alpha App access for:\n\n• viewing the Schedule;\n\n• making and cancelling eligible bookings within the weekly allowance;\n\n• viewing and maintaining the member Profile fields made available to this tier; and\n\n• viewing and managing the membership through the supported membership route.\n\nIt does not include Dashboard or WOD content, Training, Leaderboards, unrestricted class booking, or attendance-tier and lifetime-performance totals reserved for other access tiers. Staff and administrator permissions are independent of this consumer product.\n\n6. Timetable and service changes\n\nZero Alpha Fitness may make reasonable operational changes to class times, instructors, formats, facilities or the eligible timetable. It must not rely on that right to remove the essential benefit of the membership without a fair remedy. A material adverse change will be handled with reasonable notice where practicable and any cancellation, price-reduction or refund right required by law.\n\nServices must be supplied with reasonable care and skill. Nothing in this addendum excludes or restricts liability or a consumer remedy that cannot lawfully be excluded or restricted.\n",
  },
} as const;
...
export function resolveCheckoutDocuments(planKey: PlanKey): CheckoutDocument[] {
  ...
  const keys: CheckoutDocumentKey[] = [
    ...
    ...(planKey === "adult_conditioning" ?
      ["adultConditioningAddendum" as const] : []),
    "privacyNotice",
  ...
  const common: CheckoutAcceptanceStatement[] = [
    {
      ...
      statement: `I have read and agree to the Membership Terms, the Cancellation, Refund and Cooling-off Policy${planKey === "adult_conditioning" ? ", and the Adult Conditioning Only Membership Product Addendum" : ""}. I confirm that the ${plan.audience === "youth" && safeParticipantCount > 1 ? "participants'" : "participant's"} and payer or guardian details I supplied are accurate.`,
      documentKeys: [
        "membershipTerms",
        "cancellationPolicy",
        ...(planKey === "adult_conditioning" ?
    ...
    ]),
    {
      ...
      statement: familyDiscountApplies ?
        `I authorise the amount Stripe shows today and future recurring monthly payments for ${safeParticipantCount} ${plan.name} participants on the billing schedule shown at checkout. The standard total is ${formatPence(standardMonthlyPence)} per month and the automatic ${YOUTH_FAMILY_OFFER.percentOff}% family discount makes the recurring total ${formatPence(recurringMonthlyPence)} while this subscription covers at least two children. This authority is subject to my cancellation and statutory rights.` :
...
/**
 * The business owner approved this exact frozen document bundle on
 * 23 August 2026. Runtime purchase gates remain separate.
 */
...
/** Display guidance only; coaches manage youth programme placement internally. */
export function resolveYouthPlanForAge(age: number): PlanKey | null {
  if (!Number.isInteger(age)) return null;
  if (age >= 0 &&
    age < MEMBERSHIP_PLANS.youth_teenstars.minAge) return "youth_youngstars";
  if (age >= MEMBERSHIP_PLANS.youth_teenstars.minAge) return "youth_teenstars";
  return null;
}

export function isAgeEligibleForPlan(plan: MembershipPlan, age: number): boolean {
  if (!Number.isInteger(age) || age < 0) return false;
  if (plan.audience === "youth") return true;
  if (age < plan.minAge) return false;
  if (plan.maxAge !== null && age > plan.maxAge) return false;
  return true;
}

/** Age shown in the form; the callable recomputes it from the same DOB. */
...
/** Deterministic catalogue preview; Stripe remains authoritative at confirmation. */
export function resolveYouthMonthlyPricing(
  plan: MembershipPlan,
  participantCount: number
): {
  standardMonthlyPence: number;
  recurringMonthlyPence: number;
  familyDiscountApplies: boolean;
} {
  const count = Number.isInteger(participantCount) && participantCount >= 1 ?
    participantCount : 1;
  const standardMonthlyPence = plan.amountPence * count;
  const familyDiscountApplies = plan.audience === "youth" &&
    count >= YOUTH_FAMILY_OFFER.minimumParticipants;
  return {
    standardMonthlyPence,
    recurringMonthlyPence: familyDiscountApplies ?
      Math.round(standardMonthlyPence *
        (100 - YOUTH_FAMILY_OFFER.percentOff) / 100) : standardMonthlyPence,
    familyDiscountApplies,
  };
}

/** Accepts only current or explicitly grandfathered youth-family policies. */
export function isSupportedYouthFamilyDiscountPercent(value: unknown): value is number {
  return typeof value === "number" &&
    (SUPPORTED_YOUTH_FAMILY_DISCOUNT_PERCENTAGES as readonly number[]).includes(value);
}
