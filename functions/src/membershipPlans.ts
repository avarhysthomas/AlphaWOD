...
export type MembershipPlan = {
  ...
  /** Customer-facing plan name; test-mode Stripe products mirror it. */
  name: string;
  /** Display-only monthly price in pence. */
  amountPence: number;
  currency: typeof BILLING_CURRENCY;
  /**
   * Adult eligibility range or youth programme age guidance. Youth guidance is
   * deliberately not enforced at checkout; coaches manage placement internally.
   */
  minAge: number;
  maxAge: number | null;
  /**
...
/**
 * Resolves the recommended youth programme for display guidance only. The
 * result never blocks checkout; coaches manage programme placement internally.
 */
export function resolveYouthPlanForAge(age: number): PlanKey | null {
  if (!Number.isInteger(age)) return null;
  if (age >= 0 && age < MEMBERSHIP_PLANS.youth_teenstars.minAge) {
